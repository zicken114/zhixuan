...
const loadData = async (forceRefresh = false) => {
  ...
  try {
    ...
    console.error('[SentinelBrief] failed:', e);
  } finally {
    loading.value = false;
  }
};
...
自称"我"，叫用户"你"。说话要像真人，自然流畅，不要出现星号、井号、列表编号这些 markdown 符号，直接输出纯文字。`
    ...
    recordEvent({
      ...
      metadata: { title: item.title, success: true, source: 'brief', isMock: isMockData.value },
    });
  } catch (e) {
    ...
    recordEvent({
      event_type: 'hot_list_ideation',
      duration_ms: Date.now() - startTime,
      metadata: { title: item.title, success: false, source: 'brief', error: msg },
    });
  } finally {
...
const loadMaterials = async () => {
  try {
    ...
    console.error('[SentinelBrief] Failed to load materials:', e);
  }
};
...
const copyText = async (text: string) => {
  try {
    ...
    console.warn('[SentinelBrief] copy failed:', e);
  }
};
  ...
  <div class="hot-brief">
    ...
    <div class="brief-header" @mousedown="appWindow.startDragging()">
      ...
      <div class="header-actions">
        ...
        <button class="icon-btn" :disabled="loading" title="刷新" @click="loadData(true)">
          🔄
        </button>
    ...
    <div class="brief-content">
      <div v-if="loading && hotList.length === 0" class="loading">抓取热榜中...</div>
      <div v-else-if="hotList.length === 0" class="empty">
        暂无热榜数据
        <span class="empty-hint">点击刷新按钮试试</span>
      </div>
          ...
          <div v-if="expandedId === item.id" class="hot-actions">
            <div class="action-row">
              ...
              <button
                ...
                @click="quickAddToMaterial(item)"
              >
                {{ isInMaterials(item.title) ? '✓ 已加入' : '📚 加入素材库' }}
              </button>
              <button v-if="item.url" class="action-link-btn" @click="openUrl(item.url)">🔗 打开</button>
              <button v-if="item.url" class="action-link-btn" @click="copyText(item.url)">📋 复制链接</button>
            </div>
            ...
            <div v-else-if="ideationError.has(item.id)" class="ideation-error">
              ...
              <button class="retry-btn" @click="analyzeAngles(item)">重试</button>
            </div>
            ...
            <div v-else-if="ideationCache.has(item.id)" class="ideation-content">
              ...
              <div class="ideation-footer">
                <button class="action-link-btn" @click="copyText(ideationCache.get(item.id) || '')">📋 复制</button>
                <button
                  class="action-link-btn"
                  :disabled="isInMaterials(item.title)"
                  @click="addIdeationToMaterial(item)"
                >
